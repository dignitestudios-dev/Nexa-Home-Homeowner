...
export default function ReviewCard({ name, date, review, image }: ReviewCardProps) {
  return (
    <div className="relative w-full pt-10">
      ...
      {/* Card */}
      <div className="rounded-[34px] bg-[#eaf0f1] h-full px-5 pb-5 pt-2 text-black">
        <div className="mt-4">
          <h3 className="text-[20px] font-bold leading-[25px]">{name.split(" ")[0] + " " + name.split(" ")[1].slice(0, 1).toUpperCase()}</h3>
          <p className="mt-1 text-[16px]   font-semibold  text-black/70">{date}</p>
        </div>
        <p className="mt-8 text-[16px] break-all font-semibold leading-[26px] text-black/50">{review}</p>
      </div>
    </div>
  );  
}
